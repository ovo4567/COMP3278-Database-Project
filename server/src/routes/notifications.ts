...
import { listNotifications, markNotificationsReadByActor, type NotificationPayload } from '../services/notifications.js';
...
const markByActorSchema = z.object({
  actorUsername: z.string().min(1).max(100),
...
notificationsRouter.post('/read-by-actor', async (req, res) => {
  const userId = String((req as AuthedRequest).user.sub).toLowerCase();
  const parsed = markByActorSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: 'Invalid input' });

  await markNotificationsReadByActor({
    userId,
    actorUsername: parsed.data.actorUsername.toLowerCase(),
